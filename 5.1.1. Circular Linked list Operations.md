...
void traverseListInCLL(NODE head) {
    if (head == NULL) { printf("CLL is empty\n"); return; }
    NODE curr = head;
    do {
        printf("%d -> ", curr->data);
        curr = curr->next;
...
NODE insertAtPositionInCLL(NODE head, int pos , int data ) {
    ...
    if (pos > count + 1) { printf("Position not found\n"); return head; }
...
NODE deleteAtPositionInCLL(NODE head, int pos) {
    ...
    if (pos < 1 || pos > count) { printf("Position not found\n"); return head; }
    ...
    if (pos == 1) {
        ...
        if (count == 1) { free(head); return NULL; }
        tail->next = head->next;
        NODE temp = head;
        head = head->next;
        free(temp);
...
NODE reverseCLL(NODE head) {
    ...
    NODE prev = NULL, c = head, next = NULL;
    NODE tail = head;
    while (tail->next != head) tail = tail->next;
    NODE newHead = tail;
...
NODE concatCLL(NODE first, NODE second) {
    ...
    // Find tail of first CLL
    NODE tail1 = first;
    while (tail1->next != first) {
        tail1 = tail1->next;
    }

    // Find tail of second CLL
    NODE tail2 = second;
    while (tail2->next != second) {
        tail2 = tail2->next;
    }

    // Link tail of first to head of second
    tail1->next = second;

    // Link tail of second back to head of first
    tail2->next = first;

    return first; // head of concatenated CLL
